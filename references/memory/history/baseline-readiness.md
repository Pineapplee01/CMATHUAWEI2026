...
核验日期：2026-09-26。本文把“论文/源码存在”“本地可追溯”“已接入赛题数据”“已有训练证据”分开记录。源码存在不等于已经完成赛题复现；作者在完整MOSEI上的结果也不等于本题结果。
...
| L1 | EF-LSTM | MMSA有实现；MFN仓库也包含早期融合参考 | `references/MMSA/.../EF_LSTM.py`；远程 `reference/EF-LSTM/` | 独立 canonical NPZ adapter、full clean 和63条件Q2已完成 | 多种子与Q3归因验证 |
...
| L3 | MulT raw unaligned | 作者公开仓库和MMSA实现均可用 | `references/Multimodal-Transformer/`；远程 `reference/MulT/` | 原始三流长度和位置mask smoke已完成；独立 raw Q2 manifest 已实现 | full training 和raw Q2正在队列中 |
| L3 | MulT aligned_50 control | 同一MulT结构的赛题输入控制版本 | 远程 `reference/MulT/` | canonical NPZ、注意力key/query mask smoke已完成 | full training 和canonical Q2正在队列中 |
