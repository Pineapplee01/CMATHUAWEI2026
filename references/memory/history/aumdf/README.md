...
默认读取服务器预处理的 `AAAdata/Appendix_2/标准化/对齐版本/processed/{train,valid,test}.npz`，保留原
train/valid/test=3395/728/727。模型输入为768/74/35维，均为50步。`mT/mA/mV`
作为原生可用掩码传入；它们包含原始特征缺失，不能从全零值重新推断 padding。

在服务器 `Baseline/reference` 目录中，建模输入唯一允许来自上面的
`processed/{train,valid,test}.npz`；不得回退到 `reference/_data/*.pkl`。只允许向
本方法 `artifacts/` 写训练输出。输入 PKL 仅为历史单元测试兼容路径，不属于新协议。
