"""Problem 2 bridge for the mask-aware MulT implementation."""
...
class MulTAdapter:
    """Train and evaluate MulT on aligned and native unaligned processed_po views."""
    ...
    def __init__(
        ...
        *,
        ...
        if min(epochs, patience, batch_size, model_dim, heads, layers) < 1 or model_dim % heads or learning_rate <= 0:
            raise ValueError("invalid MulT training configuration")
        if not 0 <= dropout < 1:
            raise ValueError("MulT dropout must be in [0, 1)")
        ...
        self.model_dim, self.heads, self.layers, self.dropout, self.learning_rate = model_dim, heads, layers, dropout, learning_rate
    ...
    def _model_class(self):
        ...
        from e_emotion.baselines.mult import MaskedMULT
        ...
        with torch.no_grad():
            for start in range(0, split.size, self.batch_size):
                ...
                if not bool(torch.isfinite(output).all()):
                    raise FloatingPointError("non-finite MulT prediction")
    ...
    def train(self, dataset: Problem2Dataset, *, seed: int, run_dir: Path) -> Path:
        ...
        device = self._device()
        train, valid = self.reencode_text(dataset["train"]), self.reencode_text(dataset["valid"])
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for start in range(0, train.size, self.batch_size):
                ...
                if not bool(torch.isfinite(loss)):
                    raise FloatingPointError("non-finite MulT training loss")
                loss.backward()
        ...
        if not checkpoint.is_file():
            raise RuntimeError("MulT training did not produce a validation checkpoint")
        ...
        (Path(run_dir) / "mult.json").write_text(
            json.dumps({
                "method_id": self.method_id,
                "view": dataset.view,
                "model": {"model_dim": self.model_dim, "heads": self.heads, "layers": self.layers, "dropout": self.dropout},
    ...
    def predict(self, split: Problem2Split, checkpoint: Path) -> RawMethodPrediction:
        ...
        metadata = json.loads(checkpoint.with_name("mult.json").read_text(encoding="utf-8"))
        if metadata["method_id"] != self.method_id:
            raise ValueError("checkpoint belongs to a different MulT method")
...
def create_adapter() -> MulTAdapter:
    return MulTAdapter()


__all__ = ["MulTAdapter", "create_adapter"]
