...
class AUMDF(nn.Module):
    ...
    def forward(self, features, valid, observed=None):
        ...
        observed = {
            m: observed[m].to(dtype=torch.bool, device=features[m].device) & valid[m]
            for m in MODALITIES
        }
        z = {m: self.rpm[m](self.rem[m](features[m], valid[m]), valid[m]) for m in MODALITIES}
        z = self.dwam(z, valid)
        # ``valid`` retains native sequence positions for recurrent encoding;
        # ``observed`` excludes synthetic local-missing positions from pooled
        # representations and therefore from the final multimodal readout.
        modalities = {m: masked_mean(z[m], observed[m]) for m in MODALITIES}
        for block in self.blocks:
            z = block(z, valid, observed)
        fused = torch.cat([masked_mean(z[m], observed[m]) for m in MODALITIES], -1)
